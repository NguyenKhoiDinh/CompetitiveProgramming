...
void addLine(int index, long long a, long long b) {
  while (len[index] >= 2 + base[index - 1] && (B[len[index] - 2] - B[len[index] - 1]) * (a - A[len[index] - 1]) >= (B[len[index] - 1] - b) * (A[len[index] - 1] - A[len[index] - 2])) {
    len[index]--;
  }
  A[len[index]] = a;
  B[len[index]] = b;
...
long long minValue(int index, long long x) {
  ...
  while (ptr[index] + 1 < len[index] && A[ptr[index] + 1] * x + B[ptr[index] + 1] <= A[ptr[index]] * x + B[ptr[index]]) {
    ++ptr[index];
  }
  return A[ptr[index]] * x + B[ptr[index]];
}
...
int main() {
  ...
  for (int i = 0; i < MAX_T; i++) {
    ...
    if (type[i] == 1) {
      ...
      addLine(t[i], -2LL * i, C[which[i]] + 1LL * i * i);
      reached[t[i]] = true;
    }
  }
  cout << ans;
